...
(defmacro dp/with-env (command &rest body)
  (declare (indent 1))
  (let ((result (gensym "result"))
        (bufname (gensym "bufname")))
...
(defun dp/nvm (command &rest args)
  (dp/with-env (format ". ~/.profile; nvm %s %s; env" command (mapconcat (lambda (x) (format "%s" x)) args " "))
    (message (shell-command-to-string "node -v"))))

(defun dp/nvm-use (&optional version)
  (dp/nvm "use" version))

(defun dp/nvm-install (&optional version)
  (dp/nvm "install" version))

;; (defalias 'eshell/nvm 'dp/nvm)
(defalias 'eshell/nvm-use 'dp/nvm-use)
(defalias 'eshell/nvm-install 'dp/nvm-install)
