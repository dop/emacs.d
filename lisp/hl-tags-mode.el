...
(defun hl-tags-sgml-looking-at-closing-tag (point)
  (save-excursion
    (goto-char point)
    (looking-at "</")))
...
(defun hl-tags-sgml-pair (ctx)
  (if ctx
      (let* ((original-start (sgml-tag-start ctx))
             (start (+ original-start (if (hl-tags-sgml-looking-at-closing-tag original-start) 2 1)))
             (name  (sgml-tag-name ctx)))
          (cons start (+ start (length name))))
    '(1 . 1)))
